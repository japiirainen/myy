...
initialInfer :: InferState
initialInfer = InferState { count = 0 }
...
runInfer env m = runExcept $ evalStateT (runReaderT m env) initialInfer
...
-- | Canonicalize and return the polymorplic top-level type
closeOver :: Type -> Scheme
closeOver = normalize . generalize Myy.Env.empty
...
-- | Extend the type environment
isEnv :: (MonadReader Env m) => (Name, Scheme) -> m a -> m a
isEnv (x, sc) m = do
    let scope e = remove e x `extend` (x, sc)
    local scope m

-- | Lookup type in the environment
lookupEnv :: (MonadReader Env m, MonadState InferState m, MonadError TypeError m) => Name -> m Type
lookupEnv x = do
    (TypeEnv env) <- ask
...
infer :: (MonadState InferState m, MonadReader Env m, MonadError TypeError m) => Expr -> m (Type, [Constraint])
infer = \case
    ...
    Let x e1 e2 -> do
        ...
        case runSolve c1 of
            ...
            Right sub -> do
                ...
                (t2, c2) <- isEnv (x, sc) $ local (apply sub) (infer e2)
                return (t2, c1 <> c2)
...
unifies :: (MonadError TypeError m) => Type -> Type -> m Subst
...
bind :: (MonadError TypeError m) => TVar -> Type -> m Subst
